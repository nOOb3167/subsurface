...
DiveEventItem::DiveEventItem(const struct dive *d, struct event *ev, struct gasmix lastgasmix,
	...
	text(setupToolTipString(ev, lastgasmix)),
	pixmap(setupPixmap(ev, lastgasmix, pixmaps)),
	vAxis(vAxis),
	hAxis(hAxis),
	ev(ev),
	dive(d),
...
QPixmap DiveEventItem::setupPixmap(const struct event *ev, struct gasmix lastgasmix, const DivePixmaps &pixmaps) const
{
...
#ifdef SAMPLE_FLAGS_SEVERITY_SHIFT
	if (((ev->flags & SAMPLE_FLAGS_SEVERITY_MASK) >> SAMPLE_FLAGS_SEVERITY_SHIFT) == 2)
		return pixmaps.info;
	if (((ev->flags & SAMPLE_FLAGS_SEVERITY_MASK) >> SAMPLE_FLAGS_SEVERITY_SHIFT) == 3)
		return pixmaps.warning;
	if (((ev->flags & SAMPLE_FLAGS_SEVERITY_MASK) >> SAMPLE_FLAGS_SEVERITY_SHIFT) == 4)
...
QString DiveEventItem::setupToolTipString(const struct event *ev, struct gasmix lastgasmix)
{
